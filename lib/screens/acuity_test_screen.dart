...
class AcuityTestScreen extends StatefulWidget {
  const AcuityTestScreen({
    super.key,
    this.mode = vm.TestMode.distance,
  });
...
class _AcuityTestScreenState extends State<AcuityTestScreen> {
  /*
   ...
   -0.7   -> 20/4
    ...
    IMPORTANT:
    Very small levels such as 20/8, 20/5 and 20/4 are shown as
    browser-screening performance only. They should not be interpreted
    as clinically calibrated Snellen acuity without physical screen
    calibration.
  */
  ...
  double get _testDistanceCm =>
      _mode == vm.TestMode.distance ? 300 : 40;
  ...
  String _compareEyes() {
    ...
    final rightD =
        _denominator(_resultRight!);

    final leftD =
        _denominator(_resultLeft!);
  ...
  double _fontFor(
    ...
    /*
      This remains browser-based scaling.

      The physical screen size is not yet calibrated, so the numerical
      level should remain described as a screening level.
    */
    final base =
        (_mode == vm.TestMode.distance
                ? 0.095
  ...
  Future<void> _autoSaveAndGoToReport() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'Screening results saved. Opening report...',
        ),
    ...
    switch (_stage) {
      case _Stage.idleRight:
        return _CalibrationPanel(
          mode: _mode,
          title:
              'Step 1 of 3 — Right Eye (OD)',
          coverEyeText:
              'Cover your LEFT eye. Read using only your RIGHT eye.',
          buttonText:
              'Start Right Eye',
          onPressed:
              _startRight,
      ...
      case _Stage.idleLeft:
        return _CalibrationPanel(
          mode: _mode,
          title:
              'Step 2 of 3 — Left Eye (OS)',
          coverEyeText:
              'Cover your RIGHT eye. Read using only your LEFT eye.',
          buttonText:
              'Start Left Eye',
          onPressed:
              _startLeft,
